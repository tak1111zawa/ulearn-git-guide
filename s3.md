...
#### checkout – перемещение на ветку или коммит, reset – перемещение с ветки на коммит
...
- `git checkout -d <commit>` или `git switch --detach <commit>` – переместить HEAD на коммит, получится detached HEAD
- `git checkout <branch>` или `git switch <branch>` – переместить HEAD на ветку
- `git checkout -b <new_branch>` или `git switch -c <new_branch>` – создать ветку и перейти на неё
...
- `git reflog` или `git reflog show HEAD` – показать лог действий с HEAD
...
- `<ref>^N` – N-ый родитель предыдущего поколения, например, `HEAD^2`
